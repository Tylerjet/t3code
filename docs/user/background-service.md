...
Only one server can use a state directory at a time. If a terminal or desktop SSH connection
already started your server, `t3 connect` saves authorization but does not start a second server.
Service installation refuses to take over that server.
...
Do not delete ownership files to bypass a running server. A crashed server releases ownership
automatically. Servers from older releases must stop before the new server can take ownership.
